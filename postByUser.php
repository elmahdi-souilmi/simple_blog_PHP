...
<div class="container">
    ...
    <button class="btn btn-info my-4 btn-block" name="add" type="submit">Post</button>
        ...
        <div class="row">
            ...
            <!-- Post data -->
            <p>by <a><strong><?php echo $name['last_name'] . ' ' . $name['first_name'] ?></strong></a>, <?php $post['create_at'] = new DateTime();
        echo $post['create_at']->format('y-m-d');?></p>
            <!-- get the id  -->
             <?php $id = $post['id'];?>
            ...
            <a href="../delete.php?id=<?php echo $post['id'] ?>" class="btn btn-outline-danger btn-rounded waves-effect">Delete</a>
            </div>
            <!-- Grid column -->
        </div>
        <!-- Grid row -->
